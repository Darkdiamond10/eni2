...
// SHA-256 Minimal
// ============================================================================
// ... (Keeping SHA256 as it was verified correct) ...
...
// ============================================================================
// AES-256 CTR (Verified Minimal Implementation)
// ============================================================================
...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
  ...
  for (i = 32; i < 240; ++i) {
    k = (i % 32);
    tempa[0] = RoundKey[i-4]; tempa[1] = RoundKey[i-3]; tempa[2] = RoundKey[i-2]; tempa[3] = RoundKey[i-1];
...
static void Cipher(uint8_t* state, const uint8_t* RoundKey) {
  ...
  for (round = 1; round < 15; ++round) {
    ...
    for (i = 0; i < 16; ++i) state[i] = get_sbox_value(state[i]);
    ...
    if (round < 14) {
      for (i = 0; i < 4; ++i) {
        uint8_t *s = &state[i*4];
        t = s[0] ^ s[1] ^ s[2] ^ s[3];
        uint8_t tm = s[0] ^ s[1]; Tm = xtime(tm); s[0] ^= Tm ^ t;
        tm = s[1] ^ s[2]; Tm = xtime(tm); s[1] ^= Tm ^ t;
        tm = s[2] ^ s[3]; Tm = xtime(tm); s[2] ^= Tm ^ t;
        tm = s[3] ^ s[0]; Tm = xtime(tm); s[3] ^= Tm ^ t;
      }
    }
